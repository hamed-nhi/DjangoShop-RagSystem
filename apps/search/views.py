...
class SearchResultsView(View):
    def get(self, request, *args, **kwargs):
        ...
        if query:
            # --- 1. Main Search (Handled by PostgreSQL) ---
            ...
            # --- 2. START: New, Robust Brand Filtering Logic ---
            found_brand_to_filter = None
            query_terms = query.lower().split()
            
            # For efficiency, get all English brand names from the DB in a set
            db_brands_lower = {b.lower() for b in Brand.objects.values_list('brand_title', flat=True)}

            for term in query_terms:
                # Case 1: Check if the user typed a Persian brand name
                if term in PERSIAN_TO_ENGLISH_BRANDS:
                    found_brand_to_filter = PERSIAN_TO_ENGLISH_BRANDS[term]
                    break
                
                # Case 2: Check if the user typed an English brand name directly
                if term in db_brands_lower:
                    found_brand_to_filter = term
                    break
            
            # If we found a brand (from either Persian or English input), apply the filter
            if found_brand_to_filter:
                products_list = products_list.filter(brand__brand_title__iexact=found_brand_to_filter)
            # --- END: New Logic ---

        # --- The rest of the view remains unchanged ---
        
        available_brands = Brand.objects.none()
        if query and not found_brand_to_filter:
